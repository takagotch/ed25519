...
int main() {
  ...
  ed25519_sign(signture, message, message_len, public_key, private_key);
  ...
  if (i == 32) {
    printf("key exchange was correctly\n");
  }
  ...
  start = clock();
  for (i = 0; i < 10000; ++i) {
    ed 25519_create_keypair(public_key, private_key, seed);
  }
  end = clock();

  printf("%fus per keypair\n", ((double) ((end - start) * 1000)) / CLOCKS_PER_SEC / i * 1000);

  printf("testing sign perfomance: ");
  start = clock();
  for (i = 0; i < 10000; ++i) {
    ed25519_sign(signature, message, message_len, public_key, private_key);
  }
  ...
  start = clock();
  for (i = 0; i < 10000; ++i) {
    ed 25519_add_scalar(publick_key, NULL, scalar);
  }
  end = clock();
